...
def get_coco_summary(ground_truth_bbs: list[Annotation], detected_bbs: list[Annotation]):
    ...
    _bbs = _group_detections(detected_bbs, ground_truth_bbs)
...
def get_coco_metrics(
    ...
    :return:
        ...
        dict['FN']: total number of False Negative detections;

        if there was no valid ground truth for a specific class (total positives == 0),
        all the associated keys default to None apart from TP, FP, FN - they default to 0.
    """
...
def _compute_ious(dt: list[Annotation], gt: list[Annotation]):
    ...
    for g_idx, g in enumerate(gt):
        for d_idx, d in enumerate(dt):
            ious[d_idx, g_idx] = d.bbox.intersection_over_union(g.bbox)
    return ious
...
def _evaluate_image(dt: list[Annotation], gt: list[Annotation], ious, iou_threshold, max_dets=None,
    ...
    dt_sort = np.argsort([-d.confidence for d in dt], kind="stable")
    ...
    def _is_ignore(bb):
        if area_range is None:
            return False
        return not (area_range[0] <= bb.bbox.area() <= area_range[1])
